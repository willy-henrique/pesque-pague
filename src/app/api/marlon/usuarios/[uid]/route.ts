...
interface PatchBody {
  nome?: string; ativo?: boolean; senha?: string;
  permissoes?: AdminPermissao[]; setores?: string[];
}
...
export async function PATCH(
  ...
  try {
    ...
    if (!snap.exists) return Response.json({ error: "Usuário não encontrado." }, { status: 404 });
    ...
    if (body.senha !== undefined) {
      if (body.senha.length < 6) return Response.json({ error: "Senha mínima: 6 caracteres." }, { status: 400 });
      await auth.updateUser(uid, { password: body.senha });
    }
    if (role === "admin" && Array.isArray(body.permissoes)) {
      updates.permissoes = body.permissoes;
    }
    ...
    console.error("[PATCH /api/marlon/usuarios/[uid]]", err);
...
export async function DELETE(
  ...
  try {
    ...
    if (err instanceof Response) return err;
    console.error("[DELETE /api/marlon/usuarios/[uid]]", err);
